...
func (d *mockDecoder) Event() ssestream.Event { return d.current }
...
func TestParsePartialStreamEvent(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: &model.LLMResponse{Content: genai.NewContentFromText("partial", genai.RoleModel), Partial: true},
		},
		{
			name: "thinking_delta",
			raw:  `{"type":"content_block_delta","index":0,"delta":{"type":"thinking_delta","thinking":"reason"}}`,
			want: &model.LLMResponse{
				Content: genai.NewContentFromParts([]*genai.Part{{Text: "reason", Thought: true}}, genai.RoleModel),
				Partial: true,
			},
		},
		{
...
func newSSEvent(eventType, payload string) ssestream.Event {
	return ssestream.Event{Type: eventType, Data: []byte(payload)}
}
