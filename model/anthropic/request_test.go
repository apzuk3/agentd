...
func TestRequestBuilder_FromLLMRequest(t *testing.T) {
	...
	if tool.InputSchema.Type != "object" {
		t.Fatalf("tool input type not set: %v", tool.InputSchema.Type)
	}
...
func TestIsImagePart(t *testing.T) {
	tests := []struct {
		...
		{
			name: "uppercase_mime",
			part: &genai.Part{InlineData: &genai.Blob{MIMEType: "IMAGE/JPEG"}},
			want: true,
		},
		{
			name: "non_image",
			part: &genai.Part{InlineData: &genai.Blob{MIMEType: "text/plain"}},
			want: false,
		},
		{
			name: "nil_data",
